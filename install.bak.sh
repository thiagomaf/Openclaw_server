...
log "1/6: Installing Docker and Node.js 22 (LTS)..."
apt-get update -y > /dev/null
apt-get install -y curl git sudo docker.io jq ufw > /dev/null
curl -fsSL https://deb.nodesource.com/setup_22.x | bash - > /dev/null
apt-get install -y nodejs > /dev/null
success "System dependencies ready."

# 2. User Virtualization
log "2/6: Creating 'openclaw' service user..."
USER_NAME="openclaw"
...
log "5/6: Initializing user-land configuration..."
...
# 6. Environmental Persistence & Firewall
log "6/6: Configuring network and environment..."
...
if command -v ufw > /dev/null; then
    ...
    ufw allow 18789/tcp > /dev/null 2>&1 || true   # OpenClaw
    ufw --force enable > /dev/null 2>&1 || true
fi

success "Network and environment persistent."
